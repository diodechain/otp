...
%% listening to wx application events. At the moment of these are
%% all MacOSX specific events corresponding to MacNewFile() and friends: 
%% https://docs.wxwidgets.org/trunk/classwx_app.html
%%
%% * `{file_new, ""}` 
%% * `{file_open, Filename}`
%% * `{file_print, Filename}`
...
%% * `{reopen_app, ""}`
%%
%% The call always returns ok but will have sent all already received
%% events to the calling processes message queue. 
