...
#endif // wxUSE_WEBVIEW
#if wxUSE_WEBVIEW_IE
#if wxUSE_WEBVIEW_IE && wxCHECK_VERSION(3,1,3)
// wxWebViewIE::MSWSetEmulationLevel
void wxWebViewIE_MSWSetEmulationLevel(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
 wxWebViewIE_EmulationLevel level=wxWEBVIEWIE_EMU_IE11;
  ...
  while(!enif_is_empty_list(env, lstTail)) {
    if(!enif_get_list_cell(env, lstTail, &lstHead, &lstTail)) Badarg("Options");
    if(!enif_get_tuple(env, lstHead, &tpl_sz, &tpl) || tpl_sz != 2) Badarg("Options");
    if(enif_is_identical(tpl[0], enif_make_atom(env, "level"))) {
  if(!enif_get_int(env, tpl[1], (int *) &level)) Badarg("level"); // enum
    } else        Badarg("Options");
  };
  bool Result = wxWebViewIE::MSWSetEmulationLevel(level);
  wxeReturn rt = wxeReturn(memenv, Ecmd.caller, true);
  rt.send(  rt.make_bool(Result));
...
void wxWebViewIE_MSWSetModernEmulationLevel(WxeApp *app, wxeMemEnv *memenv, wxeCommand& Ecmd)
{
  ...
  while(!enif_is_empty_list(env, lstTail)) {
    if(!enif_get_list_cell(env, lstTail, &lstHead, &lstTail)) Badarg("Options");
    if(!enif_get_tuple(env, lstHead, &tpl_sz, &tpl) || tpl_sz != 2) Badarg("Options");
    if(enif_is_identical(tpl[0], enif_make_atom(env, "modernLevel"))) {
  modernLevel = enif_is_identical(tpl[1], WXE_ATOM_true);
    } else        Badarg("Options");
  };
  bool Result = wxWebViewIE::MSWSetModernEmulationLevel(modernLevel);
  wxeReturn rt = wxeReturn(memenv, Ecmd.caller, true);
  rt.send(  rt.make_bool(Result));
